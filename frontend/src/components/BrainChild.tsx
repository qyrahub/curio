...
export default function BrainChild({ childId, childName, childAge, accent, onGoDevelop }: {
  ...
  const refresh = async () => {
    ...
    const prompt = `You are "the Brain" — summarise your current read of a child from their real history only. Child: ${childName}, age ${childAge}. Inputs so far: ${inputs}.\nReviews:\n${hist || "none yet"}\nNeeds: ${needLines}.\nWrite a warm, plain-language read. Base everything ONLY on this history; if it's thin, keep it short and say what's still unknown rather than inventing. Return ONLY JSON: {"summary": string, "strengths": [string], "challenges": [string], "improving": [string], "watch": [string]}.`;
