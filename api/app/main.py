...
# ============================================================================
# Growth loop, feedback & release — durable, per-user, Mongo-backed.
# Every growth read/write is scoped to the authenticated user (user["sub"]);
# admin surfaces are gated on ADMIN_EMAILS via the JWT email claim.
# ============================================================================

GROWTH_KINDS = {
    "needs": "growth_needs",
    "reviews": "growth_reviews",
    "evaluations": "growth_evaluations",
}


def _uid(user: dict) -> str:
    uid = user.get("sub")
    if not uid or user.get("anonymous") or uid == "anonymous":
...
def growth_put(kind: str, body: dict, user: dict = Depends(get_current_user)) -> dict:
    ...
    doc["user_id"] = uid  # force scope; never trust the client's user_id
...
# ----- feedback & feature requests (any signed-in user) -----
@v1.post("/feedback")
def feedback_submit(body: dict, user: dict = Depends(get_current_user)) -> dict:
    ...
    if kind == "feedback":  # plain feedback quietly feeds the Brain
        try:
...
# ----- admin: data controls (compact / purge by age, type, status) -----
